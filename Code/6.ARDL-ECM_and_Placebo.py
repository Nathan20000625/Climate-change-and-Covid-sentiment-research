...
def ensure_us_daily_covid_death_column(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure `US_daily_covid_death` exists for ARDL-ECM exogenous controls.
    Priority:
    1) Use existing `US_daily_covid_death`
    2) Copy from common alias columns
    3) Derive from cumulative deaths by first difference
    """
    target_col = "US_daily_covid_death"
    if target_col in df.columns:
        df[target_col] = pd.to_numeric(df[target_col], errors="coerce")
        return df

    alias_candidates = [
        "US_daily_covid_deaths", "US_daily_death", "US_daily_deaths",
        "daily_covid_death", "daily_covid_deaths", "daily_death", "daily_deaths",
        "new_deaths", "new_death", "deaths_new", "deaths_daily",
    ]
    for alias in alias_candidates:
        if alias in df.columns:
            df[target_col] = pd.to_numeric(df[alias], errors="coerce")
            print(f"Created {target_col} from alias column: {alias}")
            return df

    cumulative_candidates = [
        "US_cumulative_covid_death", "US_cumulative_covid_deaths", "US_total_deaths",
        "total_deaths", "cumulative_deaths", "cum_deaths", "US_covid_death_total",
    ]
    for cum_col in cumulative_candidates:
        if cum_col in df.columns:
            cum = pd.to_numeric(df[cum_col], errors="coerce")
            daily = cum.diff()
            # Avoid negative values caused by data revisions.
            df[target_col] = daily.where(daily >= 0, 0.0).fillna(0.0)
            print(f"Created {target_col} from cumulative column: {cum_col}")
            return df

    print(f"Warning: {target_col} not found and could not be derived; it will not be used as exogenous control.")
    return df


def standardize_non_binary_columns(
    df: pd.DataFrame,
    date_col: str = None,
    binary_tol: float = 1e-12,
) -> pd.DataFrame:
    """
    对所有“非0/1（二元）”的数值列做标准化（z-score）。
    0/1 列保持不变（主要用于极端天气 dummy、响应虚拟变量等）。
    """
    df = df.copy()
    standardized_cols: List[str] = []
    binary_cols: List[str] = []
    zero_std_cols: List[str] = []

    for col in df.columns.tolist():
        if date_col is not None and col == date_col:
            continue
        # 尝试转数值；失败的列会全部变成 NaN，最终会被跳过
        ...
        is_binary = np.all(
            np.isclose(vals, 0.0, atol=binary_tol) | np.isclose(vals, 1.0, atol=binary_tol)
        )
        ...
        std_val = float(non_na.std(ddof=0))
        if pd.isna(std_val) or std_val == 0.0:
            df[col] = s_num
            zero_std_cols.append(col)
            continue

        df[col] = (s_num - mean_val) / std_val
        standardized_cols.append(col)

    print(
        f"Standardization (non-binary only) done: "
        f"standardized={len(standardized_cols)}, binary_skipped={len(binary_cols)}, zero_std_skipped={len(zero_std_cols)}"
    )
    return df
...
def zscore_series_preserve_index(series: pd.Series) -> pd.Series:
    """全样本 z-score（均值/标准差），索引与缺失位置与输入一致。

    仅用于补充「标准化水平」上的单位根报告；因果/ARDL 仍使用原始序列。
    """
    y = pd.to_numeric(series, errors="coerce").replace([np.inf, -np.inf], np.nan)
    mu = y.mean(skipna=True)
    sigma = y.std(skipna=True)
    try:
        mu_f = float(mu)
        sig_f = float(sigma)
...
def build_direction_wide_coefficients(csv_dir: str, direction: str, outfile: str = None):
    ...
    try:
        ...
        # 宽表需要包含 US_daily_covid_death 系数（用于论文表格/对比）
        ...
        def _format_val(v):
            """
            系数与标准误格式化：固定三位小数（非科学计数法）；若缺失则返回空字符串，保持单元格为空。
            """
            try:
                if pd.isna(v):
                    return ""
                ...
                return ""
            # 使用 Excel 文本公式包裹，避免 Excel 自动吞掉尾随 0（如 0.1520 显示为 0.152）
            return f'="{v:.3f}"'
        def _format_p(v):
            """
            p 值格式化：正常情况下保留 3 位小数；若缺失则返回空字符串。
            """
            try:
                if pd.isna(v):
                    return ""
                ...
                return ""
            # 同上：强制在 Excel 中按文本显示，稳定保留 4 位小数
            return f'="{v:.3f}"'
...
def export_fig3_data(output_dir: str, coint_results: List[Dict[str, object]]) -> None:
    """
    为 Figure 3 导出长表：direction, emotion, ecm/short/long 系数与 p 值（数值列，便于作图）。
    行顺序：forward 8 种情绪，再 reverse 8 种情绪（与 ecm_coefficients_summary_stacked 一致）。
    """
    try:
        os.makedirs(output_dir, exist_ok=True)
    except Exception:
    ...
    for result in coint_results:
        ...
        matched_emotion = None
        for emotion in emotions:
            if emotion.lower() in result_series1 or emotion.lower() in result_series2:
                matched_emotion = emotion
                break
        if not matched_emotion:
            continue
        ...
        if key not in result_map:
            ...
            old = result_map[key]
            old_ok = "ecm_coefficient" in old and not pd.isna(old.get("ecm_coefficient", np.nan))
            new_ok = "ecm_coefficient" in result and not pd.isna(result.get("ecm_coefficient", np.nan))
    ...
    for direction in ["forward", "reverse"]:
        for emotion in emotions:
            r = result_map.get((emotion.lower(), direction))
            if r:
                sr = r.get("short_run_coefficients", {}) or {}
                ecm_c = r.get("ecm_coefficient", np.nan)
                ecm_p = r.get("ecm_pvalue", np.nan)
                sh_c = sr.get("delta_x", np.nan)
                sh_p = sr.get("delta_x_pvalue", np.nan)
                lr_c = r.get("long_run_coefficient", np.nan)
                lr_p = r.get("long_run_pvalue", np.nan)
            else:
                ecm_c = ecm_p = sh_c = sh_p = lr_c = lr_p = np.nan
            rows.append(
                {
                    "direction": direction,
                    "emotion": emotion.upper(),
                    "ecm_coeff": _as_float(ecm_c),
                    "ecm_pvalue": _as_float(ecm_p),
                    "short_run_coeff": _as_float(sh_c),
                    "short_run_pvalue": _as_float(sh_p),
                    "long_run_coeff": _as_float(lr_c),
                    "long_run_pvalue": _as_float(lr_p),
                }
            )

    out_path = os.path.join(output_dir, "fig3-data.csv")
    try:
        pd.DataFrame(rows).to_csv(out_path, index=False, encoding="utf-8-sig")
        print(f"  Figure 3 data table saved: {out_path}")
    except Exception as e:
        print(f"  Figure 3 data export failed: {out_path}, error={e}")
...
def export_ardl_bounds_summary(output_dir: str, coint_results: List[Dict[str, object]]):
    ...
    星号规则：按 F 是否超过 I(0)/I(1) 临界值（F > 1% I(1) 上界→***，> 5% I(1) 上界→**，> 5% I(0) 下界→*）。
    """
    try:
        os.makedirs(output_dir, exist_ok=True)
    except Exception:
    ...
    def _format_f_with_bounds_stars(f_val, res) -> str:
        """
        F 统计量星号按是否超过 I(0)/I(1) 临界值判定（Pesaran 边界检验惯例）：
        F > 1% I(1) 上界 → ***；F > 5% I(1) 上界 → **；F > 5% I(0) 下界 → *。
        """
        try:
            if pd.isna(f_val):
                return ""
...
def get_f_stars(f_val):
    """
    根据 ARDL 边界检验的临界值添加显著性星号
    1% 的 I(1) 上界通常为 7.84, 5% 的上界为 4.61
    """
    try:
        f = float(f_val)
    except Exception:
    ...
    if f > 7.84:
        ...
        return f"{f:.2f}**"
    elif f > 3.17:  # 介于 I(0) 和 I(1) 之间通常算作不确定或弱显著，这里算作 *
        return f"{f:.2f}*"
    else:
        return f"{f:.2f}"
...
def build_supplementary_table_6(output_dir: str) -> None:
    """
    基于 appendix_placebo_bounds_table.csv 与 ECM 系数汇总表，
    构建 Supplementary Table 6（真实样本 vs 随机时间打乱的安慰剂样本比较）。
    """
    ...
    try:
        ...
        print(f"Supplementary Table 6 构建失败，找不到文件: {e}")
        print("请确保已完整运行稳健性检验与安慰剂分析代码。")
        return

    # 2. 处理 F 统计量 (Bounds Test)
    if "real_F" not in bounds_df.columns or "placebo_F" not in bounds_df.columns:
        print("Supplementary Table 6 构建失败：appendix_placebo_bounds_table.csv 中缺少 real_F 或 placebo_F 列。")
        return
    ...
    if {"ecm_coefficient", "ecm_pvalue"}.issubset(real_ecm.columns):
        ...
        print("real ecm 汇总表缺少 ecm_coefficient 或 ecm_pvalue 列，无法构建 Real ECT。")
        return
    ...
    if {"ecm_coefficient", "ecm_pvalue"}.issubset(placebo_ecm.columns):
        ...
        print("placebo ecm 汇总表缺少 ecm_coefficient 或 ecm_pvalue 列，无法构建 Placebo ECT。")
        return

    # 统一 ECM 表格里的情绪列大小写和方向命名
    ...
    # 调整列顺序以符合 Supplementary Table 6 的格式
    final_table = merged[
        ["direction", "emotion", "Real F-statistic", "Real ECT", "Placebo F-statistic", "Placebo ECT"]
    ].copy()

    # 排序：先展示 COVID->Climate 的 8 种情绪，再展示 Climate->COVID 的 8 种情绪
    ...
    print(f"\n✅ 成功生成 Supplementary Table 6，并已保存为: {out_file}\n")
    try:
        print("--- 表格预览 ---")
        # to_markdown 需要 tabulate 支持，如果环境中没有则捕获异常
...
def main():
    ...
    try:
        ...
        df = ensure_us_daily_covid_death_column(df)
        df = standardize_non_binary_columns(df, date_col=date_col)
        ...
        for scenario_label, shuffle_time in scenarios:
            if shuffle_time:
                print(f"\n================ Placebo 场景（随机打乱时间顺序）: {scenario_label} ================")
            ...
            # z-score 水平序列：同样的 ADF / 积分阶报告（ARDL 路径仍基于原始序列与上表积分阶）
            integ_rows_z = []
            for col in emotion_columns:
                zs = zscore_series_preserve_index(df_scen[col])
                _d_z, _diffed_z, report_z = find_integration_order(zs, f"{col}_zscore", max_d=2)
                integ_rows_z.append(report_z)
            integ_z_df = pd.DataFrame(integ_rows_z)
            front_cols_z = ["series", "final_order", "decision"]
            other_cols_z = [c for c in integ_z_df.columns if c not in front_cols_z]
            integ_z_df = integ_z_df[front_cols_z + sorted(other_cols_z)]
            output_file_z = os.path.splitext(output_file_scen)[0] + "_zscore.csv"
            integ_z_df.to_csv(output_file_z, index=False, encoding="utf-8-sig")
            print(f"Stationarity test (z-scored series) completed, saved to: {output_file_z}")
            ...
            print(f"\n=== Stationarity Test Summary (z-scored levels) ===")
            print(f"Total series: {len(integ_z_df)}")
            print(f"I(0) stationary series: {len(integ_z_df[integ_z_df['final_order'] == 0])}")
            print(f"I(1) first difference series: {len(integ_z_df[integ_z_df['final_order'] == 1])}")
            print(f"I(2) second difference series: {len(integ_z_df[integ_z_df['final_order'] == 2])}")
            ...
            # Figure 3 用长表（仅真实样本写入主 analysis_results，避免 placebo 覆盖）
            if not shuffle_time:
                export_fig3_data(output_dir_scen, coint_results)
